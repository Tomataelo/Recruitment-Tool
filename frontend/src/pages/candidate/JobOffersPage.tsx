...
export default function CandidateJobOffersPage() {
    ...
    const appliedJobOfferIds = new Set(
        myApplications?.data.map((a) => a.jobOffer?.id).filter(Boolean) ?? []
    )

    const hasCV = !!candidate?.cvFilePath

    const handleApply = async (jobOfferId: number) => {
        if (!hasCV) return
        setApplyingId(jobOfferId)
        try {
            ...
            <div className="p-4 md:p-8">
                ...
                {!isCandidateLoading && !hasCV && (
                    <div className="mb-6 px-4 py-3 bg-amber-950 border border-amber-900 rounded-xl flex items-center gap-3">
                        ...
                        <p className="text-sm text-amber-400">
                            ...
                            <a href="/candidate/profile" className="font-medium underline hover:no-underline">
                                Uzupełnij profil →
                            </a>
                        </p>
                    </div>
                ...
                {isLoading ? (
                    ...
                    <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-12 text-center">
                        <p className="text-sm text-neutral-500">Brak dostępnych ofert pracy</p>
                    </div>
                ) : (
                    <div className="space-y-3">
                        {data?.data.map((offer) => {
                            const hasApplied = appliedJobOfferIds.has(offer.id)
                                ...
                                >
                                    <div className="flex-1 min-w-0 mr-4">
                                        ...
                                        <div className="flex items-center gap-3 flex-wrap">
                                            <span className="text-xs text-neutral-500">{workModeLabels[offer.workMode]}</span>
                                            {offer.location && (
                                                <>
                                            ...
                                            <span className="text-neutral-700">·</span>
                                            <span className="text-xs text-neutral-500">min. {offer.experienceMin} mies.</span>
                                        </div>

                                        {offer.requirements && offer.requirements.length > 0 && (
                                            <div className="flex flex-wrap gap-1.5 mt-2">
                                                {offer.requirements.slice(0, 5).map((req) => (
                                                    <span
                                                        key={req.skill}
                                                ...
                                                {offer.requirements.length > 5 && (
                                                    <span className="text-xs text-neutral-600">+{offer.requirements.length - 5}</span>
                                                )}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex-shrink-0">
                                        {hasApplied ? (
                                            <div className="flex flex-col items-end gap-1">
                                                <span className="text-xs px-3 py-1.5 rounded-lg bg-green-950 text-green-400 border border-green-900 font-medium">
                                                  Aplikowano ✓
                                                </span>
                                                <span className="text-xs text-neutral-600">Już aplikowałeś</span>
                                            </div>
                                        ) : (
                                            <button
                                                onClick={(e) => {
                                                ...
                                                disabled={!hasCV || isApplying}
                                                className="px-4 py-2 text-sm font-medium bg-neutral-50 text-neutral-900 rounded-xl hover:bg-neutral-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
                                            >
                                                {isApplying ? 'Wysyłanie...' : 'Aplikuj'}
                                            </button>
                                        )}
                                    </div>
                                </div>
                ...
                {data && data.meta.pages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-6">
                        ...
                        <span className="text-xs text-neutral-500">{page} / {data.meta.pages}</span>
                        <button
